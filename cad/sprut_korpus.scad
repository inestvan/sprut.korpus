...
// !!! Страница Ozon из среды сборки недоступна (антибот). Ниже — типовой
// !!! габарит панели 10.1" 1024x600 (HSD101PWW1 / BP101WX1 / EJ101IA):
// !!! 235 x 143 x 5.5, активная область 222.7 x 125.3.
// !!! Замерь свой монитор и поправь эти числа.
// ---------------------------------------------------------------------
mon_w   = 235.0;   // ширина монитора (габарит)
mon_h   = 143.0;   // высота
mon_t   = 5.5;     // толщина панели (без платы драйвера)
view_w  = 224.0;   // окно в рамке (активная область + ~1 мм)
view_h  = 126.5;
view_dx = 0.0;     // смещение окна от центра панели (+ вправо)
view_dy = 0.0;     // (+ вверх)
drv_zone = 12.0;   // глубина за панелью под плату драйвера / шлейф / HDMI
...
pi_edge_gap = 1.0;        // край платы Pi до внутренней плоскости борта
